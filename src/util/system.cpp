...
const char * const BITCOIN_CONF_FILENAME = "sinchocoin.conf";
...
#else
    const char* pszModule = "sinchocoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Sinchocoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Sinchocoin";
#else
    // Unix
    return pathRet / ".sinchocoin";
#endif
#endif
}
