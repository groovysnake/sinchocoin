Sinchocoin Core version *0.15.1* is now available from:

  <https://download.sinchocoin.org/sinchocoin-0.15.1/>
...
A number of changes to the way Sinchocoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hugs per kB (0.00001 SCHO). This is to prepare for dropping the minimum transaction fee to 0.1 hugs per kB (0.0001 SCHO) in the next release.
